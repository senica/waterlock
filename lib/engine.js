...
module.exports = function(){
    ...
    _attachAuthToUser: function(auth, cb){
      ...
      // create the user
      if(!auth.user){
        waterlock.User.create({auth:auth.id, email: auth.email}).exec(function(err, user){
          if(err){
        ...
        .exec(function(err, auth){
          if(err){
            ...
            return cb(err);
					...
					// populate projects to user - added by senica
					if(auth.user && auth.user.id){
						User.findOne(auth.user.id).populate('projects').exec(function(err, user){
    ...
    attachAuthToUser: function(attributes, user, cb){
      ...
      waterlock.User.findOne(user.id).populate('projects').exec(function(err, user){
        if(err){
          waterlock.logger.debug(err);
          return cb(err);
        }
